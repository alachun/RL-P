...
class LowerEnv(gym.Env):
    def __init__(self, env_name, instance, dfa_text=None, accepting_state=None, action_space='box'):
        # create env and dfa
        ...
        if self.dfa is not None:
            low = np.zeros((len(obs.goal.literals) + self.dfa.node_num,), dtype=np.int8)
            high = np.ones((len(obs.goal.literals) + self.dfa.node_num,), dtype=np.int8)
            self.goal_space = Box(low=low, high=high, shape=(len(obs.goal.literals) + self.dfa.node_num,), dtype=np.int8)
            self.goal_obs = np.zeros((len(obs.goal.literals) + self.dfa.node_num,))
        else:
            self.goal_space = Box(low=0, high=1, shape=(len(obs.goal.literals),), dtype=np.int8)
            self.goal_obs = np.zeros((len(obs.goal.literals),))
        self.non_dfa_goal_num = len(obs.goal.literals)
        ...
        self.observation_space = Dict({"obs": Box(low=0, high=1, shape=(len(self.obs_tree.index_to_path) + self.dfa.node_num,),
                                         dtype=np.int8), "goal": Box(low=0, high=1, shape=(len(obs.goal.literals)+self.dfa.node_num,), dtype=np.int8)})

        # get goal index in obs
        self.goal_index = []
        for literal in obs.goal.literals:
    ...
    def dfa_trans(self, goal_obs):
        # if self.dfa is None then pass
        props = {'c1': bool(goal_obs[0]), 'c2': bool(goal_obs[1]), 'c3': bool(goal_obs[2]), 'c4': bool(goal_obs[3])}
    ...
    def step(self, action: ActType):
        ...
        for literal in obs.literals:
            index = self.obs_tree.get_index_by_path(self.get_tree_path(literal))
            obs_[index] = 1
            if literal in obs.goal.literals:
        ...
        if self.dfa is None:
            dfa_state = '0'
        if self.dfa is not None:
            dfa_terminate, if_success, dfa_state = self.dfa_trans(goal_obs)
            goal_obs[len(obs.goal.literals) + int(dfa_state)] = 1
        ...
        # reward shaping
        reward += (achieved_sub_goal_num - self.achieved_sub_goal_num)
...
if __name__ == '__main__':
    ...
    env = LowerEnv("PDDLEnvChild-snack", 0, action_space='box', dfa_text=dfa_text, accepting_state=accepting_state)
