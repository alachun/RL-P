...
def train(env_name, instance, dfa_text, accepting_state, dfa_index, training_time, log):
    ...
    agent = HAC(k_level, upper_env, lower_env, 100)
    ...
    for i_episode in range(1, max_episodes+1):
        ...
        last_state, done, info, step = agent.run_HAC(lower_env, k_level-1, copy.deepcopy(state), lower_env.final_goal, False)
        ...
        if i_episode % 10 == 0:
            ...
            data = [
                [n_steps, hour, round(sum(mean_reward) / len(mean_reward), 2), round(sum(mean_length) / len(mean_length), 2)],
            ]
            with open(log_path, 'a', newline='') as csvfile:
                csvwriter = csv.writer(csvfile)
                csvwriter.writerows(data)
...
if __name__ == '__main__':
    ...
    train("PDDLEnvChild-snack", args.i, dfa_text, accepting_state, args.d, args.t, args.l)
