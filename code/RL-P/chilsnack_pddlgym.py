...
if __name__=="__main__":
    ...
    model=MyA2C('MlpPolicy',env=env,verbose=1,tensorboard_log="./grid-visit-all_tensorboard/c15_2_3")#
    def timeout_handler(signum,frame):
        print("Time is over.")
        sys.exit(1)
    #设置定时器时间
    signal.signal(signal.SIGALRM,timeout_handler)
    signal.alarm(3420)#设置定时器
    #模拟程序运行：
    try:
