...
'''
示例
env=ChildsnackEnv(child_sum=10,bread_sum=10,content_sum=10,tray_sum=3,table_sum=3,sandw_sum=13,
                  no_gluten_bread_index=[2,9,4,8],
                  no_gluten_content_index=[2,8,4,1],
                  allergic_gluten_index=[1,10,3,4],
                  waiting_tables=[2,1,1,2,3,3,3,2,1,3],
                  served_order=[1,2,3],
                  viable_state=[3,4,5],
                  final_state=[5],
                  edges=[(1,1),(1,2),(1,3),(2,2),(3,2),(3,3),(3,4),(4,2),(4,4),(4,5),(5,5),(5,2)],
                  plan_sum=2)
'''
class ChildsnackEnv(gym.Env):
    def __init__(self,child_sum,bread_sum,content_sum,tray_sum,table_sum,sandw_sum,no_gluten_bread_index,no_gluten_content_index,allergic_gluten_index,waiting_tables,served_order,viable_state,final_state,edges,plan_sum):
        ...
        viable_state表示自动机中可以到达的节点,为列表类型,不包含初始节点q1和错误节点q2;eg:[3,4,5]
        ...
        self.observation_shape_size=bread_sum+content_sum+sandw_sum*3+child_sum+sandw_sum*tray_sum+tray_sum*(table_sum+1)+self.viable_state_node
        ...
        self.fluents_index = [0, bread_sum,bread_sum+content_sum,bread_sum+content_sum+sandw_sum,
                              ...
                              bread_sum+content_sum+sandw_sum*2+sandw_sum*tray_sum+child_sum+tray_sum*(table_sum+1),
                              bread_sum+content_sum+sandw_sum*3+sandw_sum*tray_sum+child_sum+tray_sum*(table_sum+1),]
        #定义返回的规划数量
        ...
        #三时序的状态匹配字典
        '''键表示三个时序对应的served child的值，其中1为真0为假；值表示当前DFA处于哪个状态节点'''
        self.cases={
            "000":1,
            "100":3,
            "110":4,
    ...
    def step(self, action):
        ...
        domain_file = Path("domain_3.pddl")#domain文件不会变化
        problem_file = Path("problem.pddl")
        heuristic = "ff()"#启发式算法
        start_time = datetime.now()

        #调用kstar规划器求解
        plans = planners.plan_topk(domain_file=domain_file, problem_file=problem_file,
                                   number_of_plans_bound=self.plan_sum, search_heuristic=heuristic, timeout=180)
        ...
        #执行规划解前的状态
        state_pre = self.state.copy()
        # 执行规划解前的掩码
        mask_pre = copy.deepcopy(self.valid_action)
        j = 0
        while j < len(plans['plans']):
            #规划解长度（包括trans）
            plan_len = plans['plans'][j]['cost']
            if plan_len == 0:
                break
            ...
            for i in range(plan_len):
                ...
                #动作参数
                parameters_tuple = action_split[1:]
        ...
        while j < self.plan_sum:
            '''#+action mask
            self.reward_table[state_str + action1_str + str(j)] = [state_pre.copy(), -1000, True, mask_pre.copy(),plan_len//2]'''
            ...
            if action[0]<self.child_sum:
                ...
                #无解的情况
                    ...
                    self.reward_table[state_str + action1_str + str(j)] = [state_pre.copy(),-1000,True,mask_pre.copy(),0]
            #选择的子任务为DFA状态节点
            else:
    ...
    def reset(self,**kwargs):
        ...
        #初始状态notexist都为True
        replace_indices=np.arange(m+self.tray_sum*(self.table_sum+1),m+self.tray_sum*(self.table_sum+1)+self.sandw_sum)
        state[replace_indices]=1
        #更新当前状态
        self.state=state
        #更新当前状态可选子目标
    ...
    def compute_valid_action_array(self,state,valid_action_array):
        ...
        first_dig = str(state[self.fluents_index[5] + self.served_order[0] - 1])
        second_dig = str(state[self.fluents_index[5] + self.served_order[1] - 1])
        third_dig = str(state[self.fluents_index[5] + self.served_order[2] - 1])
        match_str = first_dig + second_dig + third_dig
        if match_str in self.cases.keys():
        ...
        for i,mask in enumerate(valid_action_array):
            ...
            if subgoal_index>=self.child_sum:
                if self.viable_state[subgoal_index-self.child_sum] in self.succs[dfa_state] and (subgoal_index-self.child_sum+3)!=dfa_state:
