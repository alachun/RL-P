...
'''citycar对应的pddlgym环境（三个连续时序+3f），用于纯强化学习训练和gym环境调用（计算规划解的执行效果）
...
class WrapperEnv_citycar(gym.Env):
    ...
    def step(self,action):
        ...
        #判断第二个自动机f3到达哪个状态
        f3_state=self.compute_f3_state(self.f3_state,obs)
        self.f3_state=f3_state
        #判断回合结束时任务是否已解决
        if done==True:
            if  q2==False and self.compute_goal_complete(obs) and f3_state==4:
                reward=1000
            else:
                reward=-1000
    ...
    def compute_f3_state(self,f3_state,obs):
        #F(a & (F(b & (F c))))
        if f3_state==1:
...
if __name__=="__main__":
    ...
    env = WrapperEnv_citycar(place=[3,0,0,3,0,1])
