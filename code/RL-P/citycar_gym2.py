...
from citycar_pddlgym2 import WrapperEnv_citycar
from pddlgym.structs import Predicate,Literal
from citycar_pddl_problem2 import get_citycar_problem
'''输入的问题为problem文件'''
"四个连续时序+四个F"
'''动作空间为Discrete  
...
'''
'''示例
env=CitycarEnv(place=[2,1,3,1,1,3,2,2],
                  served_order=[1,2,3,4],
                  viable_state=[3,4,5,6,8,9,10,11],
                  final_state=[6,11],
               edges=[(1, 1), (1, 2), (1, 3), (2, 2), (3, 2), (3, 3), (3, 4), (4, 2), (4, 4), (4, 5), (5, 2), (5, 5),
                      (5, 6), (6, 6), (6, 2),
                      (7, 7), (7, 8), (7, 9), (7, 10), (7, 11), (8, 8), (8, 9), (8, 10), (8, 11), (9, 9), (9, 10),
...
class CitycarEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if combined_str in self.reward_table.keys():
            ...
            '''调用pddlgym会导致本gym环境和pddlgym环境不同步
                        ...
                        需要f4_state是因为pddlgym中更新f4对应的自动机状态时需要当前f4自动机的状态，所以需要一起同步'''
            self.pddlgym_env.set_state(self.state.copy())
            self.pddlgym_env.f4_state=self.reward_table[combined_str][4]
        ...
        forth_dig = str(self.state[self.pddlgym_env.goal_index[3]])
        match_str = first_dig + second_dig + third_dig+forth_dig
        dfa_state = self.cases[match_str]
        for index in range(10,16):#q1-q6
        ...
        for index in range(16,21):#q7-q11
            if (index-15)==self.pddlgym_env.f4_state:
                self.problem.set_initial_value(self.problem.fluents[index], True)
            else:
                self.problem.set_initial_value(self.problem.fluents[index], False)
        ...
        elif action[0] <self.pddl_goal_sum+4:#4个连续时序目标（q3,q4,q5,q6）
            self.problem.add_goal(self.problem.fluents[12+action[0]-self.pddl_goal_sum])
        else:#f4自动机目标(q8,q9,q10,q11)
        ...
        f4_pre=self.pddlgym_env.f4_state#执行plan前f4所处状态
        for plan_i in range(self.plan_sum):
            ...
            self.pddlgym_env.set_state(state_pre.copy())
            self.pddlgym_env.f4_state=f4_pre
            ...
            if result.status == PlanGenerationResultStatus.SOLVED_SATISFICING or result.status == PlanGenerationResultStatus.SOLVED_OPTIMALLY:
                ...
                for i in range(2, plan_len, 3):
                    ...
                    # 每次执行都需要判断是因为可能中间某动作car到达错误地点，但是后续动作车可能离开错误地点
                ...
                self.reward_table[state_str + action1_str + str(plan_i)] = [state_pre.copy(), -1000, True,
                                                                            mask_pre.copy(),
                                                                            self.pddlgym_env.f4_state, 0]

        # 更新当前状态的state以及valid_action
        self.state = self.reward_table[combined_str][0].copy()
        self.valid_action = self.reward_table[combined_str][3].copy()
    ...
    def compute_valid_action_array(self,state,valid_action_array):
        ...
        forth_dig = str(state[self.pddlgym_env.goal_index[3]])
        match_str = first_dig + second_dig + third_dig+forth_dig
        if match_str in self.cases.keys():
            dfa_state = self.cases[match_str]#当前4个连续时序对应的自动机所处状态
        else:
        ...
        for i,mask in enumerate(valid_action_array):
            ...
            # 4个连续时序的自动机对应的时序目标（只有当前状态的直接后继不含当前状态可被选择）
            if (self.pddl_goal_sum<=subgoal_index) and (subgoal_index<self.pddl_goal_sum+4):
                if self.viable_state[subgoal_index-self.pddl_goal_sum] in self.succs[dfa_state] and (subgoal_index-self.pddl_goal_sum+3)!=dfa_state:
                    ...
                    valid_action_array[i]=False
            # 4F的自动机对应的时序目标（只有当前状态的直接后继不含当前状态可被选择）
            if subgoal_index>=self.pddl_goal_sum+4:
                if self.viable_state[subgoal_index-self.pddl_goal_sum] in self.succs[self.pddlgym_env.f4_state+6] and (subgoal_index-self.pddl_goal_sum-2)!=self.pddlgym_env.f4_state:
                    valid_action_array[i]=True
                else:
                    valid_action_array[i]=False
